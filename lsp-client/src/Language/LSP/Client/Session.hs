...
instance (Monad m, (HasState s m)) => HasState s (ConduitM a b m) where
    get = lift get
    put = lift . put

instance (Monad m, (HasState s m)) => HasState s (ConduitParser a m) where
    get = lift get
    put = lift . put
    ...
    let context = SessionContext serverIn absRootDir messageChan timeoutIdVar reqMap initRsp config caps
        ...
        serverAndListenerFinalizer tid = do
            let cleanup
                ...
                (timeout msgTimeoutMs (runSession' exitServer))
...
updateState (FromServerMess SWorkspaceApplyEdit r) = do
  ...
  where
    ...
    mergeParams params =
        let events = concat (toList (map (toList . (^. contentChanges)) params))
